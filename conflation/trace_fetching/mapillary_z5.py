...
def run(
    ...
    bbox_sections = split_bbox_z5_tiles(
        session, sequence_ids_dir, z5_tiles, access_token, start_date_epoch
    )
...
def split_bbox_z5_tiles(
    ...
    tiles that we should call to find trace sequences. FIXME
    ...
    try:
        ...
        for tile in z5_tiles:
            ...
            resp = session_.get(COVERAGE_TILES_URL.format(COVERAGE_ZOOM, x, y, access_token_))
            if resp.status_code != 200:
                raise ConnectionError(
            ...
            bbox_sections.extend(
                z14_tiles_from_coverage_tile_to_bbox_sections_z5(
                    tile_pb,
                    start_date_epoch,
                    base_x_zoom_14,
                    base_y_zoom_14,
...
def z14_tiles_from_coverage_tile_to_bbox_sections_z5(
    tile_pb: vector_tile_pb2.Tile,
    ...
    for layer in tile_pb.layers:
        ...
        for feature in layer.features:
            ...
            for i in range(0, len(feature.tags), 2):
                ...
                # Only consider pixels where the latest sequence is recent enough
